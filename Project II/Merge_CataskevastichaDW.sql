--MERGE FOR DimEmployee--

ALTER TABLE FactSales
NOCHECK CONSTRAINT FK__FactSales__Emplo__4D94879B
GO
...
ALTER TABLE FactSales
CHECK CONSTRAINT FK__FactSales__Emplo__4D94879B
GO

--MERGE FOR Dimcustomer--

ALTER TABLE FactSales
NOCHECK CONSTRAINT FK__FactSales__Custo__4CA06362
GO
...
ALTER TABLE FactSales
CHECK CONSTRAINT FK__FactSales__Custo__4CA06362
GO

--MERGE for DimRawMaterial--

ALTER TABLE FactProduction
NOCHECK CONSTRAINT FK__FactProdu__RawMa__52593CB8
GO
...
ALTER TABLE FactProduction
CHECK CONSTRAINT FK__FactProdu__RawMa__52593CB8
GO

--MERGE for DimProduct--

ALTER TABLE FactProduction
NOCHECK CONSTRAINT FK__FactProdu__Produ__5165187F
GO

ALTER TABLE FactSales
NOCHECK CONSTRAINT FK__FactSales__Produ__4E88ABD4
GO

SELECT * FROM DimProduct;
SELECT * FROM StagingCataskevasticha.dbo.Products;

INSERT INTO DimProduct (ProductSKU, ProductName, CostPerUnit, ConstructionTimeInDays, Color, RowIsCurrent, RowStartDate, RowEndDate)
SELECT ProductSKU, ProductName, CostPerUnit, ConstructionTimeInDays, Color, 1, SYSDATETIME(), '9999-12-31'
FROM(
    MERGE DimProduct AS [target]
    USING StagingCataskevasticha.dbo.Products AS [source]
    ON target.ProductSKU = source.ProductSKU 
        WHEN MATCHED AND (source.ProductName <> Target.ProductName OR source.CostPerUnit <> Target.CostPerUnit OR source.ConstructionTimeInDays <> Target.ConstructionTimeInDays OR source.Color <> Target.Color)
            THEN UPDATE SET target.RowIsCurrent = 0, Target.RowEndDate = SYSDATETIME()
        WHEN NOT MATCHED BY TARGET 
        ...
        WHEN NOT MATCHED BY Source 
            THEN UPDATE SET target.RowEndDate = SYSDATETIME()
    OUTPUT source.ProductSKU, source.ProductName, source.CostPerUnit, source.ConstructionTimeInDays, source.Color, $Action AS ActionName
) AS [Merge] 
WHERE ActionName = 'UPDATE'
AND ProductSKU IS NOT NULL;

ALTER TABLE FactProduction
CHECK CONSTRAINT FK__FactProdu__Produ__5165187F
GO

ALTER TABLE FactSales
CHECK CONSTRAINT FK__FactSales__Produ__4E88ABD4
GO